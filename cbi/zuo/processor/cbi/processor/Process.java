...
public class Process {
	...
	private int positive; // number of passing runs
	private int negative; // number of failing runs
	
	public Process(PredicateProfile[] predicateProfiles){
	 ...
	 * 
	 */
	private void computePositiveNegative() {
		...
		assert(positive + negative == profiles.length);
	}
...
//		assert(statistics.length == 6 && statistics[0].length == 4);
	...
	private double Importance(int[] statisticData) {
		...
		if(increase < 0 || Math.abs(increase - 0) < 0.00001){
			return 0;
		}
		return (double) 2/(1/increase + Math.log(negative)/Math.log(statisticData[0]));
	}
	...
	public void printTopKPredictors(int k){
		...
		Collections.sort(list, new Comparator(){
			...
			public int compare(Object arg0, Object arg1) {
				// TODO Auto-generated method stub
				return ((Map.Entry<PredicateItem, Double>) arg0).getValue().compareTo(((Entry<PredicateItem, Double>) arg1).getValue());
			}
			});
